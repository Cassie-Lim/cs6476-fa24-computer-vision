...
class ImageLoader(data.Dataset):
    ...
    def load_imagepaths_with_labels(
        ...
        for k, v in class_labels.items():
            folder_path = os.path.join(self.curr_folder, k)
            for img in os.listdir(folder_path):
                img_paths.append((os.path.join(folder_path, img), v))
    ...
    def get_classes(self) -> Dict[str, int]:
        ...
        class_labels = [f for f in os.listdir(self.curr_folder) if not f.startswith('.')]
        class_labels.sort()
    ...
    def load_img_from_path(self, path: str) -> Image:
        ...
        # img = torch.from_numpy(np.array(Image.open(path).convert("L"))).unsqueeze(0)
