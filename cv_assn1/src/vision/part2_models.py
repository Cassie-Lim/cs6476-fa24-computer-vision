...
class HybridImageModel(nn.Module):
    ...
    def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
        ...
        ############################
        ### TODO: YOUR CODE HERE ###
        kernel = create_Gaussian_kernel_2D(int(cutoff_frequency))[None, None, :, :]
        assert hasattr(self, 'n_channels'), "'n_channels' attribute not found"
        kernel = torch.from_numpy(np.tile(kernel, [self.n_channels, 1, 1, 1])).float()
    ...
    def low_pass(self, x: torch.Tensor, kernel: torch.Tensor):
        ...
        x_pad = F.pad(x, [pad_len, pad_len, pad_len, pad_len])
        filtered_image = F.conv2d(x_pad, kernel, groups=self.n_channels)
    ...
    def forward(
        ...
        low_freq2 = self.low_pass(image2, kernel)
        high_frequencies = image2 - low_freq2
