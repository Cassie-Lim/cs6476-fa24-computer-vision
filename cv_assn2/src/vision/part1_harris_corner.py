...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # image_tensor = nn.functional.pad(image_tensor, (1, 1, 1, 1), mode='replicate')
    # Ix = nn.functional.conv2d(image_tensor, scharr_x).squeeze().numpy()
    # Iy = nn.functional.conv2d(image_tensor, scharr_y).squeeze().numpy()
    Ix = nn.functional.conv2d(image_tensor, scharr_x, padding=1).squeeze().numpy()
    Iy = nn.functional.conv2d(image_tensor, scharr_y, padding=1).squeeze().numpy()

    # Ix = nn.functional.conv2d(image_tensor, scharr_x)
    # Iy = nn.functional.conv2d(image_tensor, scharr_y)
    # Ix = nn.functional.pad(Ix, (1,1,1,1), mode='replicate').squeeze().numpy()
    # Iy = nn.functional.pad(Iy, (1,1,1,1), mode='replicate').squeeze().numpy()
    # Ix = nn.functional.pad(Ix, (1,1,1,1)).squeeze().numpy()
    # Iy = nn.functional.pad(Iy, (1,1,1,1)).squeeze().numpy()
    # print(Ix.shape, Iy.shape)
    # import cv2
    # Ix = cv2.Scharr(image_bw, cv2.CV_64F, 1, 0)
    # Iy = cv2.Scharr(image_bw, cv2.CV_64F, 0, 1)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def compute_harris_response_map(
    ...
    det_M = np.abs(S_xx * S_yy - S_xy * S_xy)
    trace_M = S_xx + S_yy
    R = det_M - alpha * (trace_M * trace_M)
    print(image_bw.shape, R.shape)
...
def nms_maxpool_pytorch(
    ...
    R[R<torch.median(R)] = 0.0

    R_numpy = R.numpy() 
    R_pool = maxpool_numpy(R_numpy, ksize)
    local_max_mask = (R_numpy == R_pool)
    local_max_values = R_numpy * local_max_mask
    flattened_values = local_max_values.flatten()
    topk_indices = np.argsort(flattened_values)[::-1][:k]
    
    y, x = np.divmod(topk_indices, R.shape[1])
    confidences = flattened_values[topk_indices]
...
def get_harris_interest_points(
    ...
    # TODO: YOUR CODE HERE                                                    #
    ###########################################################################
    R = compute_harris_response_map(image_bw, ksize=7)
    R_normalized = R /(R.max()+1e-6)
    x, y, c = nms_maxpool_pytorch(R_normalized, k, 7)
