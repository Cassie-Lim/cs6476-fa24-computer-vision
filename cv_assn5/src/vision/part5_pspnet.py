    ...
    def __replace_conv_with_dilated_conv(self):
        ...
        for name, module in self.layer3.named_modules():
            if isinstance(module, nn.Conv2d) and module.kernel_size == (3, 3):
                if module.stride == (2, 2) and module.padding == (1, 1) and module.dilation == (1, 1):
    ...
    def forward(
        ...
        if self.zoom_factor != 1:
            logits = F.interpolate(
                logits, 
                size=(math.ceil(x_size[2] * self.zoom_factor / 8), math.ceil(x_size[3] * self.zoom_factor / 8)), 
                mode='bilinear', 
                align_corners=False
            )
        ...
        if y is not None:
            aux_loss = self.criterion(self.aux(aux_output), y)
